function figure_13(fig_config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% FIGURE 13: Total flux (Q) vs. 'L'
...
for j=1:n_betas
    for i=1:n_gamma
        ...
        for s=1:n_strat
            ...
            for z=1:numel(idxs)
                Q_ = results.total_flux{idxs(z)}; %retrieve total flux Q_
                Q_ = Q_ / max(Q_); %for normalization
                L = results.n_links{idxs(z)};
                L = cumsum(-diff(L)); %number of pruned links.
                L = [0, L];
...
%EXPORT FIGURE------
export_fig(gcf, is_eps, sprintf('EVOLUTION_Total_flux_vs_L_N=%dx%d', sqrt(configs(1)), sqrt(configs(1))));
