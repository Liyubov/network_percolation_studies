function [G, removed_link] = remove_link(G, i, j)
%Remove link at indice i, j in network G

%Gather info from the nodes i & j, whose links will be removed in the subsequent code:
%Do not gather potential, as 'Potential' list is updated on the fly (ie. each time a node is removed, the corresponding potential is also removed):
%node 'i'
removed_link.degree(1) = G.degree(i);
removed_link.distance_source(1) = find_dis(G.adjm, i, G.inflow);
removed_link.distance_drain(1) = find_dis(G.adjm, i, G.outflow);
%node 'j'
removed_link.degree(2) = G.degree(j);
removed_link.distance_source(2) = find_dis(G.adjm, j, G.inflow);
removed_link.distance_drain(2) = find_dis(G.adjm, j, G.outflow);
